    ...
    public partial class Wii : EmuBase
    {
        ...
        protected override void InitActions()
        {
            ...
            foreach (var watcher in tempWatchers)
            {
                ...
                if (addrType != 0)
                {
                    ...
                    AddFakeWatcher(Watchers, watcher, new WiiDeepPointer(MEM1, MEM2, address));
                }
                else
                {
                    ...
                    AddFakeWatcher(Watchers, watcher, new WiiDeepPointer(MEM1, MEM2, absBase, offsets));
                }
            }
        ...
        private void AddFakeWatcher(FakeMemoryWatcherList FakeWatcherList, MemoryWatcher watcher, WiiDeepPointer wdpr)
        {
            switch (watcher)
            {
                ...
                case StringWatcher: int numBytes = watcher.GetField<int>("_numBytes"); FakeWatcherList.Add(new FakeMemoryWatcher<string>(() => wdpr.DerefString(game, numBytes)) { Name = watcher.Name }); break;
                default: throw new NotImplementedException();
            }
        }
